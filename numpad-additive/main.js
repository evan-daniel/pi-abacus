...
window.addEventListener('DOMContentLoaded', DOMContentLoaded => {
    ...
    let pi_running_total_left = +pi_digits[pi_cursor_left], pi_running_total_right = +pi_digits[pi_cursor_right]; 
    ...
    const handle_numpad_touchstart = (touchstart, side) => {
        if(touchstart.target.classList.contains('numpad-num')) {
            if(touchstart.target.getAttribute('value') === (side === 'left' ? pi_digits[pi_cursor_left] : (pi_running_total_right % 10).toString())) {
                side === 'left' ? ++pi_cursor_left : ++pi_cursor_right; 
                if(side === 'right') {
                    pi_running_total_right += +pi_digits[pi_cursor_right]; 
                }
                set_pi_cursors(side); 
            }
        }
